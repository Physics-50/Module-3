...
plot(x,y,'ko','MarkerFaceColor','k','LineWidth',1,'MarkerSize',7); % plot the data
...
%% perform an unweighted fit of a line of the form y= m*x + b to the data

fit_func = @(m,b,x) b+m*x;  % here, y = m*x + b (linear function); 
                           
[curve, goodness, output] = fit(x,y,fit_func);

% perform a fit of the function "fit_func" to the data (x,y) without weighting
...
% This script is intended for fitting data without error bars 
% on the individual data points. We can still estimate an
% uncertainty on the fit based on the distribution of the residuals 
