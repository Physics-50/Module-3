...
Notice how we derived (\ref{eq:simplified_young}) by measuring the distance $latex x$ between the first antinode and the central maximum. A similar equation follows by measuring the distance $latex \chi$ between the $latex m$th antinode and the central maximum.&nbsp; The only difference is that now the path difference is $latex m\lambda$ as opposed to just $latex \lambda$. In this case, the generalization of (\ref{eq:simplified_young}) is $$\frac{\chi}{L}= \frac{m\lambda}{d}. \tag{2}\label{eq:youngs}$$ Note, however, that as one proceeds further away from the central maximum, the assumption that the paths from the two slits to $latex H_i$ are parallel to each other may begin to break down.</p>
