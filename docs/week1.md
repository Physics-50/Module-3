...
Notice how we derived this equation by measuring the distance $$ x$$ between the first antinode and the central maximum. A similar equation follows by measuring the distance $$ \chi$$ between the $latex m$th antinode and the central maximum.&nbsp; The only difference is that now the path difference is $latex m\lambda$ as opposed to just $$ \lambda$$. In this case, the generalized equation is $$\frac{\chi}{L}= \frac{m\lambda}{d}. \tag{2}\label{eq:youngs}$$ Note, however, that as one proceeds further away from the central maximum, the assumption that the paths from the two slits to $$ H_i$$ are parallel to each other may begin to break down.
