...
*[Click here to open in a new tab](https://docs.google.com/forms/d/e/1FAIpQLSeF85mLdADYHRDn3v3ciPKl8TPqTIbpI7X9c-1N7DUKFsP9PA/viewform?usp=sf_link){:target="_blank"}*
...
we also have that $latex \tan \theta \approx \tan\beta = x / L$. Let us return to the schematic we used in pre-lab 1 to make a more accurate approximation.
...
In pre-lab 1 we argued that since the screen is very far away compared to the spacing between slits $latex \alpha \approx 90^{\circ}$. This approximation continues to be valid. However, if we continue to decrease our slit spacing, d then we might and should question the approximation $latex \sin \theta \approx \tan\theta$. A quick look at Figure 1 should convince us that we can do better. Slit 1, Point A and slit 2 form a right triangle. The hypotenuse is d and we argued in last week's pre-lab that the length of the segment between point A and slit 2 is $latex \lambda$. Using the Pythagorean theorem this implies $latex \tan\theta = {\lambda}/{\sqrt{d^2-\lambda^2}}$. Substituting this expression for $latex \tan\theta $ into our analysis of prelab #1  gives us <strong id="youngs">Young's equation</strong>, $$\frac{x}{L}= \frac{\lambda}{d\sqrt{1 - \lambda^2/d^2}}. \tag{1}\label{eq:young}$$
